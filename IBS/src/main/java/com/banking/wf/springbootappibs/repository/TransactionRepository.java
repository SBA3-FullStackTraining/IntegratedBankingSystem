...
public interface TransactionRepository extends JpaRepository<Transaction, Long>{
	...
	@Query(value = "SELECT * FROM Transaction  where customer_id =:id and last_transation_date BETWEEN :startDate AND :endDate", nativeQuery = true)
	List<Transaction> findBydate(@Param("id") Long id,@Param("startDate") String startDate,@Param("endDate") String endDate);
	
	@Query(value = "SELECT * FROM Transaction  where customer_id =:id and last_transation_date like %:year1% or last_transation_date like %:year2% ", nativeQuery = true)
	List<Transaction> findByyear(@Param("id") Long id,@Param("year1") String year1,@Param("year2") String year2);
